...
public class Program
{
    static async Task Main(string[] args)
    {
        ...
        {
            var average = 0.0;
            foreach (int j in data[i].Ranks)
            {
                average += j;
            }
            Console.WriteLine($"{data[i].Name} Average: {average / data[i].Ranks.Count} ");
        ...
        dAverage = dAverage / data[3].Ranks.Count;
        var lowestAverage = Math.Min(Math.Min(aAverage, bAverage), Math.Min(cAverage, dAverage));
        //set the winner variables to true for the group with the lowest average
        if (lowestAverage == aAverage)
        {
            aWinner = true;
        ...
        if (aWinner)
            {
            length = data[0].Ranks.Count;
        }
        else if (bWinner)
        {
            length = data[1].Ranks.Count;
        }
        else if (cWinner)
        {
            length = data[2].Ranks.Count;
        }
        else if (dWinner)
        {
            length = data[3].Ranks.Count;
        }
        //redo the averages for each group but only use the length determined by the winning group
        aAverage = 0.0;
        for (int i = 0; i < length; i++)
        {
            aAverage += data[0].Ranks[i];
        }
        aAverage = aAverage / length;
        bAverage = 0.0;
        for (int i = 0; i < length; i++)
        ...
        for (int i = 0; i < length; i++)
        {
            ...
            if (data[3].Ranks[].IndexOf(data[3].Ranks[i]) >= length)
            {
                dAverage += data[3].Ranks[i];
            }
            else
        ...
        Console.WriteLine($"Class D New Average: {dAverage}");
        //determine the new winner
        lowestAverage = Math.Min(Math.Min(aAverage, bAverage), Math.Min(cAverage, dAverage));
        aWinner = false;
        bWinner = false;
        cWinner = false;
        dWinner = false;
